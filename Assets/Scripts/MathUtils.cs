...
public class MathUtils
{
   ...
   /*light*/ {0.5f, 1f, 2f, 1f}
    ...
    public static List<Vector2Int> GetStarTiles(int range, Vector2Int centerPosition){
        int max_x = Gameboard.TILE_COUNT_X;
        int max_y = Gameboard.TILE_COUNT_Y;
        ...
        for (int i = 1; i <= range; i++)
            for (int d = 0; d < directions.GetLength(0); d++) 
            {
                ...
                if (CheckMustSkipTile(tilePosition, centerPosition)) continue;
                availableTiles.Add(tilePosition);
            }
        return availableTiles;
    }
    public static List<Vector2Int> GetAreaTiles(int range, Vector2Int centerPosition)
    {
        int max_x = Gameboard.TILE_COUNT_X;
        int max_y = Gameboard.TILE_COUNT_Y;
        List<Vector2Int> availableTiles = new List<Vector2Int>();
    ...
    private static bool CheckMustSkipTile(Vector2 tilePosition, Vector2Int centerPosition)
    {
        int max_x = Gameboard.TILE_COUNT_X;
        int max_y = Gameboard.TILE_COUNT_Y;
